...
// ─────────────────────────────────────────────────────
// AddLicenseModal — foydalanuvchi o'zi litsenziyasini kiritadi
// ─────────────────────────────────────────────────────
...
function AddLicenseModal({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const list: LicenseTypeOption[] = res.data?.results || [];
        setTypes(list);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!f) return;
    // 5 MB limit — client-side ham tekshirib turadi
  ...
  const submit = async () => {
    setErr(null);
    // Client-side majburiy maydonlar tekshiruvi
    if (!typeCode) {
