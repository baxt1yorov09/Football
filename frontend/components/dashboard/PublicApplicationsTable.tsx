...
import { FileText, Search, AlertCircle, ChevronLeft, ChevronRight, Filter, Clock, Eye, CheckCircle, XCircle } from 'lucide-react';
...
const STATUS_OPTIONS = [
  { value: '', label_uz: 'Barchasi', label_ru: 'Все' },
  { value: 'pending', label_uz: 'Kutilmoqda', label_ru: 'Ожидает' },
  { value: 'under_review', label_uz: "Ko'rib chiqilmoqda", label_ru: 'На рассмотрении' },
  { value: 'approved', label_uz: 'Tasdiqlangan', label_ru: 'Одобрено' },
  { value: 'rejected', label_uz: 'Rad etilgan', label_ru: 'Отклонено' },
];
...
export function PublicApplicationsTable() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const r = await fetch(`/api/applications/public/?${params.toString()}`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
      if (!r.ok) {
      ...
      <CardHeader className="flex flex-col gap-4">
        <div className="flex flex-row items-center justify-between gap-3 flex-wrap">
          <CardTitle className="text-lg font-bold text-[#0D3B6E] flex items-center gap-2">
            ...
            {locale === 'ru' ? 'Все заявки' : 'Barcha arizalar'}
          </CardTitle>
          {data && (
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder={locale === 'ru'
                ? 'Поиск по имени, типу лицензии...'
                : 'Ism, litsenziya turi bo\'yicha qidirish...'}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {STATUS_OPTIONS.map((opt) => (
              ...
              >
                {locale === 'ru' ? opt.label_ru : opt.label_uz}
              </button>
            ))}
          </div>
                ...
                {loading ? (
                  Array.from({ length: 5 }).map((_, i) => (
                    <tr key={i} className="border-b border-gray-100 animate-pulse">
                      ...
                      <td className="py-3 pr-3"><div className="h-7 w-10 bg-gray-200 rounded" /></td>
                      <td className="py-3 pr-3"><div className="h-5 w-20 bg-gray-200 rounded-full" /></td>
                      ...
                      <td className="py-3 pr-3 hidden md:table-cell"><div className="h-3 w-12 bg-gray-100 rounded" /></td>
                    </tr>
                  ))
                ) : rows.length === 0 ? (
                  <tr>
                  ...
                  rows.map((app, idx) => {
                    const localizedTypeName = locale === 'ru'
                      ? (app.license_type_name_ru || app.license_type_name)
                      ...
                      <tr key={app.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        ...
                        <td className="py-3 pr-3">
                          <span
                            ...
                            style={{ backgroundColor: app.color_hex }}
                            title={localizedTypeName}
                          >
                            {app.license_type_code.replace('_', '-')}
                          </span>
                        </td>
                        <td className="py-3 pr-3">
                          <span
                          ...
                          {app.queue_position != null ? (
                            <span
                              className={`text-xs font-medium ${
                                app.status === 'approved' ? 'text-[#27AE60]' : 'text-[#F39C12]'
                              }`}
                            >
                              {app.queue_position} / {app.queue_total}
                            </span>
                          ) : (
